...
static NSString * const HR_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const HRRouterParameterURL = @"HRRouterParameterURL";
NSString *const HRRouterParameterCompletion = @"HRRouterParameterCompletion";
NSString *const HRRouterParameterUserInfo = @"HRRouterParameterUserInfo";
...
@implementation HRRouter

+ (instancetype)sharedInstance
{
    static HRRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(HRRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        HRRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[HRRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[HRRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    HRRouter *router = [HRRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL];
    HRRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(HRRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(HRRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(HRRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url
{
    ...
    parameters[HRRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:HR_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:HR_ROUTER_WILDCARD_CHARACTER];
        }
    }
